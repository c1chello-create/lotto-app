(function(global){
  ...
  function shell(){
    return `<section class="reverse-card" id="reverseInferenceCard"><div class="reverse-head"><b>🔄 +1회 가상출현 역산</b><span>Fusion AI</span></div><p class="combo-guide">후보 조합이 다음 회차에 1회 출현했다고 가정해 모든 연결 점수를 다시 계산하고 목표점수에 가장 적은 교체로 도달하는 조합을 찾습니다.</p><div class="reverse-controls"><label>목표 Fusion<select id="reverseTarget"><option>80</option><option>85</option><option selected>90</option><option>95</option></select></label><label>최대 교체<select id="reverseMax"><option>1</option><option selected>2</option><option>3</option></select></label></div><p class="combo-guide"><b>Fast Gate 80점 · +1 Fast v4</b> · 최고 가능점수가 80점 미만인 후보는 Pattern 정밀계산 전에 제외합니다. Fusion 계산식은 변경하지 않습니다.</p><button type="button" class="combo-btn" id="reverseRun">+1회 빠른 역산 실행</button><div id="reverseResult"><p class="combo-guide">실제 lotto.json은 변경하지 않습니다.</p></div><div class="reverse-precise"><div class="reverse-precise-head"><b>🎯 목표점수 정밀 역산</b><span>25개 완전탐색 · Fast v4</span></div><p class="combo-guide">확정된 25개 후보풀 안에서 선택한 최대 교체 수까지 모든 조합을 검사합니다. 목표점수에 도달하면 해당 조합을 모두 집계하고, 도달 조합이 없으면 실제 최고점 TOP10을 확정합니다.</p><div class="reverse-controls"><label>도달 목표<select id="reversePreciseTarget"><option>85</option><option>90</option><option selected>95</option><option>98</option></select></label><label>정밀 최대 교체<select id="reversePreciseMax"><option>1</option><option>2</option><option selected>3</option></select></label></div><button type="button" class="combo-btn" id="reversePreciseRun">🎯 목표점수 정밀 역산</button><div id="reversePreciseResult"><p class="combo-guide">25개 후보풀을 확정한 뒤 실행하세요. 목표 95점·최대 교체 3개일 때 22,059개 조합을 검사합니다.</p></div></div></section>`;
  }
  function attach(){
  ...
  async function run(){
    ...
    try{const r=await global.ReverseInferenceEngine.run({target:Number(document.getElementById('reverseTarget')?.value||90),maxReplace:Number(document.getElementById('reverseMax')?.value||2),cutoff:80,onProgress:t=>progress(t)});render(r);}finally{btn.disabled=false;btn.textContent='+1회 빠른 역산 실행';}
  }
