(function(global){
  ...
  function render(r){
    ...
    const shortlist=sl&&sl.nums?.length?`<div class="reverse-shortlist"><div class="reverse-shortlist-head"><b>🎯 2차 압축 번호군</b><small>총 ${sl.nums.length}개</small></div><div class="reverse-shortlist-row">${(sl.core||[]).map(n=>`<span class="core">핵심 ${n}</span>`).join('')}${(sl.support||[]).map(n=>`<span class="support">보조 ${n}</span>`).join('')}</div><div class="reverse-shortlist-note">Fast Gate 생존 번호가 많을 때 핵심 8개를 우선 유지하고, 8위와 점수차가 작은 후보만 보조로 최대 4개 추가합니다. 이후 정밀 역산은 이 번호군을 중심으로 진행합니다.</div></div>`:'';
