(function(global){
  ...
  function shell(){
    return `<section class="reverse-card" id="reverseInferenceCard"><div class="reverse-head"><b>🔄 +1회 가상출현 역산</b><span>Fusion AI</span></div><p class="combo-guide">후보 조합이 다음 회차에 1회 출현했다고 가정해 모든 연결 점수를 다시 계산하고 목표점수에 가장 적은 교체로 도달하는 조합을 찾습니다.</p><div class="reverse-controls"><label>목표 Fusion<select id="reverseTarget"><option>80</option><option>85</option><option selected>90</option><option>95</option></select></label><label>최대 교체<select id="reverseMax"><option>1</option><option selected>2</option><option>3</option></select></label></div><button type="button" class="combo-btn" id="reverseRun">+1회 역산 실행</button><div id="reverseResult"><p class="combo-guide">실제 lotto.json은 변경하지 않습니다.</p></div></section>`;
  }
  function attach(){
  ...
  function render(r){
    ...
    const b=r.best;
    const stages=r.stages.map(s=>`<div><span>${s.replaceCount}개 교체 · ${s.count}개 평가</span><b>${s.best?`${s.best.after}점 · 목표달성 ${s.met}개`:'후보 없음'}</b></div>`).join('');
