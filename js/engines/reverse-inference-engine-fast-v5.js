(function(global){
  ...
  async function run(opts={}){
    ...
    try{
      ...
      onProgress(`1단계 후보를 ${cutoff}점 생존 커트라인으로 선별합니다${fastReady?' · Fast v5':''}...`);
      ...
      const result={
        ...
        engineVersion:fastReady?'plus1-fast-v5':'legacy',
        ...
        note:`Fusion 계산식은 변경하지 않았습니다. ${cutoff}점은 1차 연산 중단용 커트라인입니다. ${fastReady?'Fast v5 경로는 목표점수 정밀역산과 같은 사전 Pattern 컨텍스트·안전 상한·후보별 +1 증분계산을 사용합니다. ':''}1단계가 끝나면 생존 번호를 점수·반복·상승폭으로 다시 평가해 핵심 8개와 보조 최대 4개, 총 8~12개로 압축합니다. 이후 단계는 압축된 번호군만 계산하므로 후보 수가 줄어들수록 연산량도 함께 줄어듭니다.`
      };
      state.last=result;return result;
